...
int simular_ap(int estado, int idx_input, const char *input, char *pilha, int topo, int passos) {
    ...
    // Varre todas as transições registradas para o estado atual
    for (int i = 0; i < qtd_transicoes_ap[estado]; i++) {
        TransicaoAP t = transicoes_ap[estado][i];

        // Verifica se o símbolo de entrada bate (ou se a transição aceita lambda '\')
        int cond_input = (t.lido == '\\') || (input[idx_input] != '\0' && input[idx_input] == t.lido);
        
        // Verifica se o topo da pilha bate (ou se desempilha lambda '\' que não exige nada)
        int cond_pilha = (t.desempilha == '\\') || (topo > 0 && pilha[topo - 1] == t.desempilha);

        if (cond_input && cond_pilha) {
            // Criar ramificação da pilha para não contaminar os outros caminhos (Backtracking)
            char nova_pilha[1000];
            memcpy(nova_pilha, pilha, topo);
            int novo_topo = topo;

            // Se removeu algo que não era lambda, desempilha
            if (t.desempilha != '\\') {
                novo_topo--;
            }

            // Se tem algo para empilhar (e não é lambda '\')
            if (strcmp(t.empilha, "\\") != 0) {
                int len = strlen(t.empilha);
                // Empilha de trás para frente para manter a ordem correta no topo
            ...
            // Avança o ponteiro da fita apenas se consumiu caractere real (não-lambda)
            ...
            if (simular_ap(t.destino, proximo_idx, input, nova_pilha, novo_topo, passos + 1)) {
                return 1; // Sucesso em alguma ramificação!
            }
        }
    }

    return 0; // Este caminho falhou
}

// --- FUNÇÃO PRINCIPAL DE PROCESSAMENTO ---
void processar_arquivo(const char *nome_arquivo) {
    char linha[256];
    int modo_automato = 0; // 0 = AF, 1 = MT, 2 = ALL, 3 = AP
    ...
    while (fgets(linha, sizeof(linha), arquivo)) {
        ...
        if (strncmp(linha, "@MT", 3) == 0) { modo_automato = 1; }
        else if (strncmp(linha, "@ALL", 4) == 0) { modo_automato = 2; }
        else if (strncmp(linha, "@AP", 3) == 0) { modo_automato = 3; }
        else if (strncmp(linha, "@AF", 3) == 0) { modo_automato = 0; }
                    ...
                    if (modo_automato == 1 || modo_automato == 2) {
                        ...
                        if (virgula && barra_trans) {
                            ...
                            *barra_trans = '\0';
                            ...
                            // Restaura os ponteiros do token
                            *virgula = ',';
                            *barra_trans = '/';
...
// --- MAIN COM 3 ARQUIVOS ---
int main() {
    processar_arquivo("entrada1.txt"); // Testa MT
    processar_arquivo("entrada2.txt"); // Testa ALL
    processar_arquivo("entrada3.txt"); // Testa o novo AP adicionado!
